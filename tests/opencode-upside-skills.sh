...
[[ "${GIT_SSH_COMMAND:-}" == *"ControlMaster=auto"* ]] || {
  printf 'Git SSH command does not enable connection sharing\n' >&2
  exit 2
}
[[ "$GIT_SSH_COMMAND" == *"ControlPersist=yes"* ]] || {
  printf 'Git SSH command does not persist the shared connection\n' >&2
  exit 2
}
[[ "$GIT_SSH_COMMAND" == *"ControlPath="* ]] || {
  printf 'Git SSH command does not set a control socket\n' >&2
  exit 2
}
git_ssh_command_record="$CLAUDE_STUB_STATE/git-ssh-command-$PPID"
if [[ -s "$git_ssh_command_record" ]]; then
  [[ "$(cat "$git_ssh_command_record")" == "$GIT_SSH_COMMAND" ]] || {
    printf 'Git SSH command changed during one refresh\n' >&2
    exit 2
  }
else
  printf '%s\n' "$GIT_SSH_COMMAND" >"$git_ssh_command_record"
fi

git_config_index=$((${GIT_CONFIG_COUNT:?} - 1))
git_config_key="GIT_CONFIG_KEY_${git_config_index}"
git_config_value="GIT_CONFIG_VALUE_${git_config_index}"
[[ "${!git_config_key:-}" == "url.git@github.com:.insteadOf" ]] || {
  printf 'missing GitHub SSH URL rewrite key\n' >&2
  exit 2
}
[[ "${!git_config_value:-}" == "https://github.com/" ]] || {
  printf 'missing GitHub SSH URL rewrite value\n' >&2
  exit 2
}
...
# GNU timeout creates a separate process group by default. The production
# wrapper must request foreground mode because Claude's plugin CLI accesses the
# controlling terminal even for some non-interactive commands.
cat >"$TEST_ROOT/timeout" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

[[ "${1:-}" == "--foreground" ]] || {
  printf 'timeout was not invoked with --foreground\n' >&2
  exit 2
}
shift
[[ "${1:-}" =~ ^[0-9]+$ ]] || {
  printf 'timeout was not given a numeric duration\n' >&2
  exit 2
}
shift
exec "$@"
EOF
chmod +x "$TEST_ROOT/timeout"

cat >"$TEST_ROOT/ssh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

printf '%s\n' "$*" >>"$CLAUDE_STUB_STATE/ssh-calls"
EOF
...
export TIMEOUT="$TEST_ROOT/timeout"
export SSH="$TEST_ROOT/ssh"
...
bash "$SCRIPT" refresh
grep -Eq '^-S .+ -O exit git@github.com$' "$STATE/ssh-calls" \
  || fail 'shared SSH connection was not closed after refresh'
