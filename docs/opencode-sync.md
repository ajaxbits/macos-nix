...
explicit marketplace and plugin commands. Ensure the work account can access
`upside-services/ai-marketplace` without an interactive prompt. For HTTPS,
configure the GitHub CLI credential helper:

```sh
gh auth status
gh auth setup-git
git ls-remote https://github.com/upside-services/ai-marketplace.git HEAD
```

The generated command environment includes `git` and `gh`, but it does not
embed credentials or load interactive shell startup files. Authentication
errors are logged and retried at the next run.
