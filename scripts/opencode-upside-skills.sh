...
cleanup() {
  ...
  if [[ -n "$ssh_control_path" ]]; then
    "$SSH" -S "$ssh_control_path" -O exit git@github.com >/dev/null 2>&1 || true
  fi
...
make_ssh_control_dir() {
  if [[ -z "$ssh_control_dir" ]]; then
    ssh_control_dir="$(mktemp -d "${TMPDIR:-/tmp}/oc-us.XXXXXX")"
    ssh_control_path="$ssh_control_dir/s"
    printf -v git_ssh_command \
      '%q -o ControlMaster=auto -o ControlPersist=yes -o ControlPath=%q' \
      "$SSH" "$ssh_control_path"
  fi
}
...
run_claude() {
  ...
  if [[ ! "$git_config_count" =~ ^[0-9]+$ ]]; then
    log "GIT_CONFIG_COUNT must be a non-negative integer"
    return 1
  fi

  make_ssh_control_dir

  # Claude's plugin CLI may access the controlling terminal even for JSON
  # commands. GNU timeout normally puts its child in a separate process group,
  # which causes an interactive shell to stop Claude with SIGTTOU. Keep Claude
  # in the foreground process group while retaining the command deadline.
  #
  # Upside's marketplace manifests publish HTTPS GitHub URLs, while Git access
  # on these machines is deliberately authenticated over SSH via Secretive.
  # Add a process-local Git rewrite so Claude's nested clones use that existing
  # Git authentication without coupling them to the separate gh token.
  env \
    "GIT_CONFIG_COUNT=$((git_config_count + 1))" \
    "GIT_CONFIG_KEY_${git_config_count}=url.git@github.com:.insteadOf" \
    "GIT_CONFIG_VALUE_${git_config_count}=https://github.com/" \
    "GIT_SSH_COMMAND=$git_ssh_command" \
    "$TIMEOUT" --foreground "$COMMAND_TIMEOUT_SECONDS" "$CLAUDE_PLUGIN_CLI" "$@"
}
